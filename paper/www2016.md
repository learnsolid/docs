...
社会网络的重新分散是一个重要的课题， 也是一个活跃的研究领域。solid 平台是社交 web 应用程序分散平台的具体实例，它提供分散身份验证、分散的数据管理，并用库和 Web 组件的形式提供开发人员支持，以及一套运行服务器和示例应用程序。本 Demo 将展示 solid 平台如何启用社交应用程序，同时允许每个用户保留对其 pod 的控制。Demo 参与者将从用户和应用程序开发人员的角度体验 solid。他们将深入了解 solid 提供的互操作性和可移植性功能、它可以启用的丰富社交功能以及这些功能背后的客户端和服务器机制。对这一平台的具体认识在正在进行的关于重新权力下放的讨论中非常有价值。
